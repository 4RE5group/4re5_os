#!/bin/bash

OUTPUT="./bin"

PATH="$(pwd)/toolchain/bin:$PATH"

OS_NAME="4re5_os"
VERSION="1.0.0"
FINAL_FILE="$OS_NAME-$VERSION.bin"

COLOR_ERROR="\e[31m"
COLOR_GREEN="\e[32m"
COLOR_RESET="\e[0m"

# Clear OUTPUT directory
rm -rf $OUTPUT/*
mkdir -p $OUTPUT
clear

echo "COMPILING ASM BOOTLOADER"
nasm -fbin bootloader/boot.asm -o $OUTPUT/boot.bin || exit 1
nasm -felf kernel/entry.asm -o $OUTPUT/entry.o || exit 1

echo "COMPILING C KERNEL"
i686-linux-gnu-gcc -fno-pic -ffreestanding -m32 -g -c kernel/main.c -o $OUTPUT/kernel.o || exit 1

echo "LINKING WHOLE OS"
i686-linux-gnu-ld -m elf_i386 -Ttext 0x1000 --oformat binary -nostdlib \
    -o $OUTPUT/fullkernel.bin $OUTPUT/entry.o $OUTPUT/kernel.o || exit 1

echo "MOUNTING IMG ==> $FINAL_FILE"
cat $OUTPUT/boot.bin $OUTPUT/fullkernel.bin > "$FINAL_FILE" || exit 1


qemu-system-i386 -net nic,model=rtl8139 -net user -drive format=raw,file="$FINAL_FILE"
